...
check_files ()
{
    ...
    for f in ${runname}.{sites,mdp,pbp} $GROin $TOPin $GroDIR
      do
      if [ ! -f $f ]; then
    ...
    #
    # Check directories
    for d in $MeadDIR $MToolsDIR $PetitDIR $StDIR $DelphiDir
      do
      if [ ! -d $d ]; then 
          message E  "Directory $d does not exist!!!... Program will crash"
      fi
    done    
    if [[ ($CpHModule == redox || $CpHModule == dendrimer || $CpHModule == delphi) && ! -f $RULEin ]]; then
        message E  "File $RULEin is missing!!!... Program will crash"
    fi

    if [[ $PBdim -ne 0 && $PBdim -ne 2 ]]; then        message E  "Number of dimensions ($PBdim) is not supported in ${CpHModule} module!!!... Program will crash"
    fi
    #
    ##
    ###
    ### Debug of joined CpHMD at 14/01/2021 by noliveira ###
    ###
    ##
    #
    ...
    if [[ $scheme == "verlet" ]]
    then
        message W "Cutoff used is verlet."

    elif [[ $scheme == "group" ]]
    then
        message W "Cutoff used is Group based."
    else
        message W "Cutoff not group nor verlet... Performing method as group based cut off"
...
check_nchains () #SCAMPOS: new function to account for several chains #15-11-2011
{
    ...
    if [[ $PDBin == *(" ") ]]; then
        nchains=1
        message W  "Empty variable PDBin. One single chain considered."
    else
        if [ -f $PDBin ]; then
            nchains=`awk 'BEGIN{chain_old=" "};/ATOM/{chain_now=substr($0,22,1); if (chain_now != chain_old) {chain_old=chain_now; if (chain_now != " ") n++}}END{print n}' $PDBin`
            ...
            message E "File $PDBin is missing!!!... Program will crash"
        fi
    fi
}
...
set_termini()
{
    ...
    if [[ $nchains -gt 1 ]]; then
        ...
        NTres=`awk 'BEGIN{chain_old=" "};/ATOM/{chain_now=substr($0,22,1); if (chain_now != chain_old) {new_chain=1; chain_old=chain_now}; if (chain_now != " ") if (new_chain && $3=="N"){new_chain=0; print substr($0,23,4)}}' $PDBin`
        CTres=`awk 'BEGIN{chain_old=" "};/ATOM/{if ($3 == "C") last_c=substr($0,23,4); chain_now=substr($0,22,1); if (chain_now != chain_old) {if (chain_old != " ") print last_c; chain_old=chain_now}}END{if (chain_old != " ") print last_c}' $PDBin`
...
run_PBMC()
{
	...
	#
	#    "$MToolsDIR"/makepqr W 2RT TMP_pqr.top TMP_pqr.top \
