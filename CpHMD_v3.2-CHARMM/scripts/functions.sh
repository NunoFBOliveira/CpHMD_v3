...
build_forcefield ()
{
    ...
    ln -s -f "$ffDIR"/${ffID}.ff .; ln -s -f "$ffDIR"/*.dat .
}
...
make_delphi_DB()
{
	...
	awk  '$2~"CT" {printf"%-6s%-9s%6.3f\n", $1,$2,$3}' $DelphiDir/DataBaseT_${ffID}.${type} >> ./DataBaseT.${type}
	awk  '$2~"NT" {printf"%-6s%-9s%6.3f\n", $1,$2,$3}' $DelphiDir/DataBaseT_${ffID}.${type} >> ./DataBaseT.${type}
	##
	...
	for db_res in `awk '/ATOM/ {print substr($0,18,2)}' TMP_solute.pdb | sort | uniq`;do	
		...
		awk -v d=$db_res '$2~d {printf"%-6s%-9s%6.3f\n", $1,$2,$3}' $DelphiDir/DataBaseT_${ffID}.${type} >> ./DataBaseT.${type}
	    else
		message E  "Residue identifier $db_res is not on Delphi Database_${ffID}. Program will crash"
	    fi	
	done
    done
...
check_nchains () #SCAMPOS: new function to account for several chains #15-11-2011
{
    ...
    if [[ $nchains -gt 1 ]]; then 
 	Ctres=`awk 'BEGIN{chain_old=" "};/ATOM/{if ($3 == "C") last_c=substr($0,23,4); chain_now=substr($0,22,1); if (chain_now != chain_old) {if (chain_old != " ") ; chain_old=chain_now ; n++ ; if (n!=1) print last_c+0 }}END{if (chain_old != " ") print last_c+0}' TMP_protein.pdb`
    else
	Ctres=`awk ' /ATOM/ {if ($3 == "C") last_c=substr($0,23,4);}END{print last_c+0}' TMP_protein.pdb`
    fi
    
    for l in $Ctres ;do
	if [[ $ffID == "G54a7pH" ]] ;then
	    sed -i "/ $l /s/ O1 / O  /" TMP_protein.pdb
	elif [[ $ffID == "CHARMM36pH" ]] ;then
	    sed -i "/ $l /s/ OT1 / O   /" TMP_protein.pdb
	elif [[ $ffID == "Amber14SBpH" ]] ;then
	    sed -i -e "/ $l /s/ OC1 / O   /" -e "/ OC2.*$l /d" TMP_protein.pdb
	fi
    done
}
...
set_termini()
{
	...
	message W  "Name of Nterminus titrating is: ${NTres}."
        message W  "Name of Cterminus titrating is: ${CTres}."
        ...
        #Attribute state to the Nter and Cter not titrating
         ...
         # Capping Conditions
                if [[ $Titrating == "FALSE" ]]; then #not titrating
		    if [[ $ffID == "G54a7pH" ]] ;then
			case ${Nterminus[$n]} in
                            CAP )           NTer[$i]=6 ;;
                            CAPpro )        NTer[$i]=11 ;; 
			...
			case ${Nterminus[$n]} in 
                            CAP )           NTer[$i]=7 ;;
                            CAPpro )        NTer[$i]=11 ;; #needs rechecking 
                            CHG )           NTer[$i]=3 ;;
			    #CHGpro)         NTer[$i]=2 ;;
                            NEU | TAU1 )    NTer[$i]=0 ;;
                            TAU2 )          NTer[$i]=1 ;;
...
#################################################################
    ...
    # Capping Conditions
		...
		case ${Nterminus} in
                    CAP )           nter=7 ;;
                    CAPpro )        nter=11 ;; #needs rechecking 
                    CHG )           nter=3 ;;
		    #CHGpro)         nter=2 ;;
                    NEU | TAU1 )    nter=0 ;;
                    TAU2 )          nter=1 ;;
...
#14-11-2011
        ...
        if [[ ${NTindex[@]} != "" ]]; then
            for i in ${NTindex[@]}
            do
		## add routine for when nres tit add 4 to NTer{$1]
