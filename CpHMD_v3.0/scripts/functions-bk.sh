...
{                   
    ...
    offset=2000 # which bypasses the user defined value (older settings)
...
check_files ()
{
    ...
    for f in ${runname}.{sites,mdp,pbp} $GROin $TOPin $GroDIR $PDBin $NDXin
      do
      if [ ! -f $f ]; then
    ...
    #
    # Check directories
    for d in $MToolsDIR $PetitDIR $StDIR $DelphiDir
      do
      if [ ! -d $d ]; then 
...
##     cp -f ${runname}.sites ${runname}-all.sites
    ...
    sed -i "s/\(nsteps *= \).*\$/\1 $EffectiveSteps/" ${runname}.mdp
    #
    ...
    "$GroDIR" grompp -f TMP_effective.mdp -po TMP_effective_out.mdp \
        -c  $PDBin -p $TOPin -pp TMP_processed.top -n TMP_CpHMD.ndx \
        -o TMP_CpHMD.tpr -maxwarn 1000 -quiet
...
set_termini()
{
    ...
    if [[ $nchains -gt 1 ]]; then
        ...
        CTres=`awk 'BEGIN{chain_old=" "};/ATOM/{if ($3 == "C") last_c=substr($0,23,4); chain_now=substr($0,22,1); if (chain_now != chain_old) {if (chain_old != " ") print last_c; chain_old=chain_now}}END{if (chain_old != " ") print last_c}' TMP_protein.pdb`
...
## 	#
## 	#    "$DelphiDir"/makepqr W 2RT TMP_pqr.top TMP_pqr.top \
...
##         "$DelphiDir"/makepqr W 2RT "$CpHDIR"/top/ff${ffID}nb.itp $TOPin \
## 		    TMP_aux1.gro > TMP_aux.pqr
## 	#fi
##     else
        if [ ${PBdim} -eq 2 ]; then
...
##             "$DelphiDir"/stmodels $offset TMP_aux.pqr ${runname}.sites
...
##                 "$DelphiDir"/stmodels $offset TMP_aux.pqr ${runname}-all.sites
...
##         if [[ $1 != red && $RTThreshold != 0 ]]; then 
##             "$DelphiDir"/statepqr s=TMP_statepqr.in $pqr_file \
...
##         "$DelphiDir"/statepqr r=c TMP_${runname}.pqr ${runname}.sites \
##             > ${runname}.pqr
##     #
##     # Run MeadT and Multiflex:
##         "$DelphiDir"/meadT -n $nCPU -s "$DelphiDir" -m "$MeadDIR" \
...
#################################################################
    ...
    # Run PDB2GMX to generate the new topology
        ...
        "$CpHDIR"/scripts/pdbswitch 1 TMP_MCarlo_std.out \
