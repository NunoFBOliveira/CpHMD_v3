...
build_forcefield ()
{
    ...
    cp -rf "$CpHDIR"/top/${ffID}.ff .; cp -rf "$CpHDIR"/top/*.dat .
}

check_files ()
{
    # Check for other necessary files (filenames must be respected)
    for f in ${runname}.{sites,mdp,pbp} $GROin $TOPin
      do
      if [ ! -f $f ]; then
...
run_PBMC()
{
    ...
    # (moleculetype for the solvent cannot exist) SC 13-12-2011
	    ...
	    if [[ $nchains -gt 1 ]]; then
		#
		# In case multiple objects, use fixgro/box!
		echo -e "Protein\nProtein" | $GroDIR trjconv \
						       -f TMP_effective.gro -s TMP_CpHMD.tpr \
						       -o TMP_effective_aux1.gro -n TMP_CpHMD.ndx \
						       -pbc mol -center
		#
		"$CpHDIR"/scripts/fixbox TMP_effective_aux1.gro \
...
run_dynamics ()
{
    $GroDIR grompp -f TMP_$1.mdp -po TMP_$1_out.mdp \
        ...
        -n TMP_CpHMD.ndx -o TMP_$1.tpr -maxwarn 10000 -quiet

    $mdrun -s TMP_$1.tpr -x TMP_$1.xtc -c TMP_$1.gro \
        -e TMP_$1.edr -g TMP_$1.log -o TMP_$1.trr -quiet # SC-14-12-2011
